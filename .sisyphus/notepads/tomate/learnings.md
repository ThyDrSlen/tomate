...
- Background entrypoints in WXT export `defineBackground(() => { ... })`, but tests must explicitly call the exported `main` function after import because `defineBackground` only wraps the callback.
- `fakeBrowser.runtime.sendMessage` in `wxt/testing` resolves promise return values from `onMessage` listeners; using the callback-style `sendResponse` path returns the listener's boolean instead of the intended payload in tests.
